...
pfnet_pfdata_stats = {
    ...
    "case14_seeds": {
        ...
        "mean": {
            "bus": {
                "x": torch.tensor([3.2607e-01, -2.5127e-33,  1.0821e-01,  5.3179e-02,  0.0000e+00, 1.3571e-02]),
                "y": torch.tensor([0.9873, -0.3662,  0.3484,  0.1733,  0.0000,  0.0136])
            },
            ("bus", "branch", "bus"): {
                "edge_attr": torch.tensor([0.0615, 0.2007, 0.0113, 0.9940, 0.0000])
            }
        },
        "std": {
            "bus": {
                "x": torch.tensor([0.4766, 0.0000, 0.2936, 0.0869, 0.0000, 0.0490]),
                "y": torch.tensor([0.0400, 0.1837, 0.8972, 0.3526, 0.0000, 0.0490])
            },
            ("bus", "branch", "bus"): {
                "edge_attr": torch.tensor([0.0597, 0.1078, 0.0188, 0.0163, 0.0000])
            }
        }
    }
}

pfnet_pfdata_stats_constraint_violation = {
    "case118_seeds": {
        "casename": "case118_seeds", 
        "mean": {
            "bus": {
