...
class PFDeltaDataset(InMemoryDataset):
    def __init__(
        ...
        self.perturbation = perturbation  # none, n-1, n-2
        self.sample_type = (
            sample_type  # feasible, near_infeasible, approaching_infeasible
        )
        ...
        if task in [3.2, 3.3]:
            ...
            # if empty just parse the data for the given case and topological perturbation
            self._custom_processed_dir = os.path.join(
                self.root,
                "processed",
                f"task_{self.task}_{self.case_name}_{self.perturbation}_{self.sample_type}",
            )  # maybe it makes sense to have a different folder name for this
            self.split = "all"
        else:
            self._custom_processed_dir = os.path.join(
        ...
        self.task_config = {  # values here will have the number of train samples
            ...
            "analysis": {
                "feasible": {"none": 56000, "n-1": 29000, "n-2": 20000},
                "near infeasible": {"none": 2000, "n-1": 2000, "n-2": 2000},
                "approaching infeasible": {"none": 7200, "n-1": 7200, "n-2": 7200},
            },
        }

        if case_name == "case2000":  # has to change maybe?
            self.task_config = {
    ...
    def processed_file_names(self):
        if self.task == "analysis":
            return ["all.pt"]  # Only require pt for analyssi
    ...
    def get_analysis_data(self, root):  # note that root here is the case folder
        dataset_size = self.task_config[self.task][self.sample_type][self.perturbation]
        data_list = []

        # TODO: could make this is a dict instead somewhere in the constructor.
        if self.sample_type == "feasible":
            raw_fnames = [
                os.path.join(root, self.perturbation, "raw", f"sample_{i + 1}.json")
                for i in range(dataset_size)
            ]

        elif self.sample_type == "near infeasible":
        ...
        elif self.sample_type == "approaching infeasible":
            raw_fnames = glob.glob(
                os.path.join(root, self.perturbation, "around_nose", "train", "*.json")
            )

        data_list = []
        feasibility = True if self.sample_type != "feasible" else False
            ...
            for grid_type in ["none", "n-1", "n-2"]:
                ...
                if feasibility == "feasible":
                    shuffle_path = os.path.join(
                        root, "shuffle_files", grid_type, "raw_shuffle.json"
                    )
                    ...
                    # extend the lists instead of overwriting (don't think this is implemented?)
                    split_dict = {
    ...
    def process(self):
        ...
        if task in [3.1, 3.2, 3.3]:
            ...
            casename = (
                self.case_name if task == 3.1 else ""
            )  # why not include this in the last else?
        elif task == "":
            roots = [os.path.join(self.root, self.case_name, self.perturbation)]
        ...
        if task == "analysis":  # no need to combine anything here
            ...
            task_data_list = self.get_analysis_data(roots[0])  # only one root here
    ...
    def load(self, split):
        ...
        if "separate" in split:  # why is this here?
