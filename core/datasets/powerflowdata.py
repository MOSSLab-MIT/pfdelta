...
@registry.register_dataset("powerflowdata")
class PowerFlowData(InMemoryDataset):
    ...
    def _normalize_dataset(
        ...
        # 6 for:
        mean = torch.mean(xy, dim=0).unsqueeze(dim=0).expand(data.x.shape[0], 6)
        std = (
            torch.std(xy, dim=0).unsqueeze(dim=0).expand(data.x.shape[0], 6)
        )  # Vm, Va, Pd, Qd, Gs, Bs
        ...
        # for edge attributes
        mean = (
            torch.mean(data.edge_attr, dim=0)
            .unsqueeze(dim=0)
            .expand(data.edge_attr.shape[0], data.edge_attr.shape[1])
        )
        std = (
            torch.std(data.edge_attr, dim=0)
            .unsqueeze(dim=0)
            .expand(data.edge_attr.shape[0], data.edge_attr.shape[1])
        )
    ...
    def raw_file_names(self) -> List[str]:
        if self.case != "mixed":
            return [
                "case" + f"{self.case}" + "_" + name for name in self.partial_file_names
            ]
        else:
            return [
                "case" + f"{case}" + "_" + name
    ...
    def process(self):
        ...
        raw_paths_per_case = [
            ...
            for i in range(0, len(self.raw_paths), 3)
        ]
        data_list = []
