...
class PowerBalanceLoss:
    """
    Computes the power balance loss.

    Args:
        model (str): String identifier for the model type
            (e.g., "CANOS", "PFNet", or "GNS"). This is used internally to correctly parse and
            interpret specific model outputs.

    Call Args:
        outputs: Model outputs containing bus predictions (voltages, angles, power injections, etc.).
        data (HeteroData): Graph data object containing bus, branch, and generator information, 
                            along with connectivity and attributes.

    Returns:
        torch.Tensor: The mean power balance loss (PBL Mean), computed as the average magnitude 
                        of squared active and reactive power mismatches across all buses.
    """
    ...
    def __call__(self, outputs, data: HeteroData):
