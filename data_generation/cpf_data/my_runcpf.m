...
if ~done.flag
    ...
    while ~done.flag
        ...
        if ~isempty(intermediate_results.order.branch.status.off)
        intermediate_results.branch(intermediate_results.order.branch.status.off, [PF QF PT QT]) = 0;
        end
        if size(intermediate_results.gen, 2) < 21
            intermediate_results.gen(:, end+1:21) = NaN;
        end
