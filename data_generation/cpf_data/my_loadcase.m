function [baseMVA, bus, gen, branch, areas, gencost, info] = my_loadcase(casefile)
...
if info == 0
    ...
    if ~( isfield(s,'baseMVA') && isfield(s,'bus') && ...
        ...
        %% === PATCH: fill flow cols and set to 0 if inactive ===
        required_branch_cols = 17;  % up to QT
        if size(mpc.branch, 2) < required_branch_cols
            mpc.branch(:, end+1:required_branch_cols) = 0;
        end

        % Fill PF, QF, PT, QT = 0 for inactive branches
        [~, ~, ~, ~, ~, ~, ~, ~, ~, ~, BR_STATUS, PF, QF, PT, QT, ~, ~, ~, ~] = idx_brch;
        inactive = (mpc.branch(:, BR_STATUS) == 0);
        mpc.branch(inactive, [PF, QF, PT, QT]) = 0;
        %% === END PATCH ===
    end
end
